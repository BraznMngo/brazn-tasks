...
| The rename sends exactly `{organization_id, organization_name, idempotency_key}`, key defaulted | **A reversed negative, recorded as such (BRA-1439 Story 2).** This row used to pin that NO rename-organization export existed — ruling C8.1's "read-only until POST /v1/organizations/rename lands" made the absence the testable property. The route landed with BRA-1439's commercial half, so the positive counterpart took over. Mutation now: remove the key default, or add any field to the body — `toEqual` pins the whole body. The descriptor's `renamed` affirmative is this page's reading of an outcome union the producer has not declared yet; the OPERATIONS row says so and fails closed on everything else |
...
| A released group releases its descendants too — but NOT one carrying `is-refused`/`data-deny-reason` itself | Delete the `querySelectorAll` loop from `releaseControl` (the release half); drop its skip (the ownership half). **Corrected claim — the fifth wrong mutation sentence found here.** This row used to say the ownership half protects `rename-org`. It never did: `refusedGroup()` puts both markers on the WRAPPER and leaves the button carrying `aria-disabled` alone, so the fixture was a shape no view emits and the real control was released on every render. The row below is the one that covers it |
| A markup refusal survives the release of a gate AROUND it | Narrow `releaseControl`'s skip back to the child's own attributes. Measured on a running build, not reasoned: for an organization administrator the Organization section's `admin` gate passes on every render, and the pencil beside the organization name lost `aria-disabled` while keeping its refusal styling and its sentence — handed to a screen reader as an ordinary available button, indistinguishable from the working team pencil next to it. The same line covers Create a team and Add seats when the seat meter cannot be read; the two refusals inside the Add-member modal were never affected, having no gated ancestor. The paired control — a plain button in the same section, which IS released — reddens on a skip that swallows everything |
